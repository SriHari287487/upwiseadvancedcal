...
export async function getBusinessHours() {
  ...
  try {
    const resp = await ZOHO.CRM.CONNECTION.invoke(conn_name, req_data);
    const businessHours = resp?.details?.statusMessage?.business_hours || null;
    
    if (businessHours) {
      return {
        enabled: businessHours.enabled || false,
        timezone: businessHours.timezone || "UTC",
        days: businessHours.business_days || {}, // { "Monday": { start_time: "09:00", end_time: "18:00" }, ... }
      };
    }
...
export function isWithinBusinessHours(startDateTime, endDateTime, businessHours) {
  if (!businessHours || !businessHours.enabled || !businessHours.days) {
    return true; // If business hours not configured, allow all times
  }
  ...
  // Get business hours for start and end days
  const startDayHours = businessHours.days[startDayName];
  const endDayHours = businessHours.days[endDayName];

  // If either day is not a business day, return false
  if (!startDayHours || !endDayHours) {
    return false;
  }

  // Parse time strings (format: "HH:MM" or similar)
  const parseTime = (timeStr) => {
    const [hours, minutes] = timeStr.split(":").map(Number);
    return hours * 60 + minutes; // Convert to minutes since midnight
  };

  const startTimeMin = parseTime(startDayHours.start_time);
  const startTimeMax = parseTime(startDayHours.end_time);
  const endTimeMin = parseTime(endDayHours.start_time);
  const endTimeMax = parseTime(endDayHours.end_time);
  ...
  // Check if event start is within business hours
  const startWithinHours = eventStartMin >= startTimeMin && eventStartMin < startTimeMax;
  
  // Check if event end is within business hours
  const endWithinHours = eventEndMin > endTimeMin && eventEndMin <= endTimeMax;
